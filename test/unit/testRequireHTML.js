...
describe('require_html.htl', () => {
  beforeEach('Run Parcel programmatically on require_html.htl', (done) => {
    const bundler = new Bundler('./test/example/require_html.htl', options);
    bundler.bundle().then(() => done());
  });

  it('correct output files have been generated', () => {
    assert.ok(fs.existsSync('./dist/require_html.js'), 'output file has been generated');
    assert.ok(!fs.existsSync('./dist/require_html.htl'), 'input file has been passed through');
  });

  it('script can be required', () => {
    logger.debug(`found generated file ${require.resolve('../../dist/require_html.js')}`);

    // eslint-disable-next-line import/no-unresolved, global-require
    const script = require('../../dist/require_html.js');
    assert.ok(script);
  });
